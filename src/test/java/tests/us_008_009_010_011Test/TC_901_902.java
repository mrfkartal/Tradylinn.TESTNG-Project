...
public class TC_901_902 extends TestBaseRapor {
    TradylinnNPage tradylinnPage;
    @Test
    public void test01() {
         tradylinnPage=new TradylinnNPage();
        extentTest=extentReports.createTest("US09","Shipping opsiyonu");
        ...
        //8-Vendor shipping butonunu tiklar
        ...
        tradylinnPage.shippingButonu.click();
        extentTest.info("Shipping butonu tiklandi");

        //9 vendor urunun Weight (kg) girer
        tradylinnPage.weight.sendKeys("15");
        ReusableMethods.waitFor(3);
        extentTest.info("urunun Weight (kg) degeri girildi");


        //10-vendor urunun Dimensions (cm) boyutlarini
        // (lengh ,width,heigth) girer

        tradylinnPage.length.clear();
        tradylinnPage.length.sendKeys("20");
        extentTest.info("urunun length degeri girildi");

        tradylinnPage.width.clear();
        tradylinnPage.width.sendKeys("15");
        extentTest.info("urunun width degeri girildi");

        tradylinnPage.height.clear();
        tradylinnPage.height.sendKeys("20");
        extentTest.info("urunun height degeri girildi");

        //11-vendor Shipping class da urunun teslimat suresini secer
        Select select1=new Select(tradylinnPage.teslimatZamani);
        List<WebElement> teslimatZamaniOptions=select1.getOptions();
        for (WebElement teslimatZamani:teslimatZamaniOptions
             ) {teslimatZamani.click();

        }
        extentTest.info("vendor Shipping class da urunun teslimat suresi secildi");
